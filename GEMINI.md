...
- **Source of Truth:** All new strings must be added to the `*strings_en.json` file. The `grunt modulify` command will then automatically update the `*strings_en.json` and `*Strings.ts` files.
- Strings in `*strings_en.json` follow a nested structure that must be mirrored in `*Strings.ts`
- The `grunt modulify` command automatically updates the `*Strings.ts` when strings are changed
...
*   **Adding Custom Simulation Preferences:**
    ...
    *   For sim-specific preferences, create a `*Preferences.ts` file in `js/common/model/` to hold the preference properties.

*   **Session Learnings:**
    *   The `grunt type-check` command needs to be run from the specific simulation's directory.
    *   `TSoundPlayer` is an interface that requires `play` and `stop` methods. If a sound generator is not meant to be controlled by the sound manager directly, it should not implement this interface.
    *   It's more efficient to create a single sound player instance for a node that manages multiple sound-producing objects, rather than creating a sound player for each object.
